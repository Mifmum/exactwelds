export const faqGroups = [
  {
    title: 'CAPABILITIES',
    items: [
      {
        question: 'What welding processes does Exact Welds specialize in?',
        answer: 'Exact Welds provides technical expertise in MIG (GMAW) and Stick (SMAW) welding. Our primary focus is on precision aluminum, stainless steel, and high-carbon steel fabrications for industrial clients in the greater Toledo region.'
      },
      {
        question: 'Are you AWS certified for structural work?',
        answer: 'Yes. Our lead technicians are AWS D1.1 Certified. Every weld performed by Exact Welds adheres to rigorous ASTM standards to ensure maximum structural integrity and safety for our Ohio manufacturing partners.'
      },
      {
        question: 'Can you handle mobile or on-site welding projects?',
        answer: 'Absolutely. Exact Welds maintains a fully equipped mobile welding unit capable of field repairs and on-site installations across Toledo and the surrounding industrial parks.'
      },
      {
        question: 'Can you weld aluminum and stainless on-site?',
        answer: 'Yes. Our mobile rig carries the shielding gas and filler stock needed for both aluminum and stainless jobs throughout Northwest Ohio.'
      },
      {
        question: 'Can you send me a quote from a photo?',
        answer: 'Yes. Upload photos in our quote form and Exact Welds will return a preliminary estimate for most Toledo-area jobs within 24 hours.'
      }
    ]
  },
  {
    title: 'PRICING',
    items: [
      {
        question: 'How do you calculate job estimates?',
        answer: 'Exact Welds provides transparent estimates based on material cost, labor hours, and job complexity. Most quotes for Toledo-area work include a breakdown of every line item before we begin.'
      },
      {
        question: 'Is there a minimum project fee?',
        answer: 'Yes. Exact Welds maintains a minimum service call fee to cover mobilization of the mobile rig across Northwest Ohio. The minimum is waived for repeat commercial customers.'
      },
      {
        question: 'Do you offer emergency repair pricing?',
        answer: 'Yes. Emergency welding in Toledo and the surrounding industrial parks is quoted on a priority basis with a same-day response fee.'
      },
      {
        question: 'Do you offer financing or payment plans?',
        answer: 'Yes. Exact Welds works with local financing partners for qualifying projects across Northwest Ohio. Ask for details when requesting your quote.'
      }
    ]
  }
];
